...
void PEQSclass::PrepareSolver()
{
  ...
  VIload(BCX,9*NPoints[0],-1);
  VIload(BCY,9*NPoints[1],-1);
  VIload(BCZ,9*NPoints[2],-1);
  BCX_mg = QAllocINTEGER(factorX);VIload(BCX_mg,factorX,-1);
  BCY_mg = QAllocINTEGER(factorY);VIload(BCY_mg,factorY,-1);
  BCZ_mg = QAllocINTEGER(factorZ);VIload(BCZ_mg,factorZ,-1);
  if(mg_order > 1){

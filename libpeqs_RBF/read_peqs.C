...
// Read the $peqs and $peqs_grid input sections
  ...
  if(is){
    ...
    do{
       ...
       if(!Done && TL.NSignifTokens())
       {
         ...
         // --- vdW Radii --- //
         if(TL[0] == "vdw_radii")
         { // these input options are defined in rem_values.h
           if(TL[1] == "Bondi")     rem_write(BONDI, REM_PEQS_VDW_RADII);
           else if(TL[1] == "UFF")  rem_write(UFF,   REM_PEQS_VDW_RADII);
           else if(TL[1] == "READ") rem_write(READ,  REM_PEQS_VDW_RADII);
           else 
           {
              printf("\n\tUnrecognized option for keyword VDW_RADII\n");
              QCrash("Please check $peqs input section");
           }
         }
         ...
         if(TL[0] == "print")
         {
           ...
           rem_write(peqs_print,REM_PEQS_PRINT);
         }
...
void PEQSclass::Read_PEQSgrid()
{
  ...
  if(is){
    ...
    do{
       ...
       {
         if (TL[0] == "DimX"){
           // JMH (5/2023): these format checks are now repeated in libgen/CheckREMs.C
           if (!TL[1].IsINTEGER() || !TL[2].IsDouble() || !TL[3].IsDouble())
              QCrash("Improper format for DimX input in $peqs_grid\n");
           NPoints[0] = TL[1].GetINTEGER();
           XMin[0] = TL[2].GetDouble()*ang2bohr;
           XMax[0] = TL[3].GetDouble()*ang2bohr;
           //cout << "is multiple of 8? " << NPoints[0]%8 << endl;
         } 
         if (TL[0] == "DimY"){
           if (!TL[1].IsINTEGER() || !TL[2].IsDouble() || !TL[3].IsDouble())
              QCrash("Improper format for DimY input in $peqs_grid\n");
           NPoints[1] = TL[1].GetINTEGER();
           XMin[1] = TL[2].GetDouble()*ang2bohr;
           XMax[1] = TL[3].GetDouble()*ang2bohr;
           //cout << "is multiple of 8? " << NPoints[1]%8 << endl;
         }

         if (TL[0] == "DimZ"){
           if (!TL[1].IsINTEGER() || !TL[2].IsDouble() || !TL[3].IsDouble())
              QCrash("Improper format for DimZ input in $peqs_grid\n");
           NPoints[2] = TL[1].GetINTEGER();
           XMin[2] = TL[2].GetDouble()*ang2bohr;
           XMax[2] = TL[3].GetDouble()*ang2bohr;
           //cout << "is multiple of 8? " << NPoints[2]%8 << endl;
         }
       }
    }while(!Done);
  }

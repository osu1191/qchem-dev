...
void PEQSclass::Read_PEQS()
{
  ...
  if(is){
    ...
    do{
	   ...
	   else if(TL[1] == "SCCS") // Solvent aware
             CavityType = PEQS_CAVTYPE_SCCS;    // Solvent aware
	 ...
	 if(TL[0] == "MaxThresh") //Gygi
           rho_max = TL[1].GetDouble(); //Gygi
         if(TL[0] == "MinThresh") //Gygi
           rho_min = TL[1].GetDouble(); //Gygi
         if(TL[0] == "MaxIter")
           max_iter = TL[1].GetINTEGER();

//========Solvent Aware====================//
	 if(TL[0] == "SolventRadius")
           Rsol = TL[1].GetDouble();
	 if(TL[0] == "FillZero")
           FillZero = TL[1].GetDouble();
	 if(TL[0] == "Scale")
           alfa = TL[1].GetDouble();
	 if(TL[0] == "zhi_switch")
           zhi_delta = TL[1].GetDouble();
	 if(TL[0] == "eta_switch")
           eta_delta = TL[1].GetDouble();
	 if(TL[0] == "ConvoLim")
           limit = TL[1].GetINTEGER();
//==========================================//

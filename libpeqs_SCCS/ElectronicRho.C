...
void PEQSclass::ElectronicRho(double *PAv)
{
  ...
  if (NTotPts <= MaxPPer) MaxPPer = NTotPts;
  else if ((MaxPPer % Nz) != 0) MaxPPer = MaxPPer - (MaxPPer % Nz);
  for (ijk=0;ijk<NTotPts;ijk+=MaxPPer){
  if (MaxPPer <= (NTotPts-ijk)) NPPer = MaxPPer;
    else NPPer = (NTotPts-ijk);
    VRload(ESP,NPPer,0.0);
    VRcopy(BatchPts,&XYZ[(3*ijk)],3*NPPer);
    AOints(ESP,NULL,NULL,NULL,PAv,BatchPts,NULL,NULL,&NPPer,12);
    VRcopy(&phi_elec[ijk],ESP,NPPer);
  }

...
double PEQS_Main(double *PAv)
{
  ...
  if (PEQS.ConvPEQ()){
    ...
    cout << "GOING into SlveEnergy()" << endl;
      double G_solvation = PEQS.SolvEnergy();
    cout << "Came out of SolveEnergy()..now in Peqs_Main()" << endl;
    //cout << " --------------------------------------- " << endl;
      return G_solvation;
    }
  }
  QCrash(" Sorry, the Poisson equation solver was unable to find a solution.");
  return 0.0;
}
